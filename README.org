...
#+begin_src d2 :file devflow.svg
    ...
    #+end_src
** ascii image
用 ~#+begin_src text~ 标记 ASCII 图，导出 HTML 时会自动套用中英等宽字体（见 ~org_css.css~ 中的 ~pre.src-text~ 规则）。若仍错位，可安装 [[https://github.com/be5invis/Sarasa-Gothic][更纱黑体 Sarasa Mono SC]]，或在块前加 =#+ATTR_HTML: :class ascii-art=。
